function plot_traj_gnd(detn, conf_bounds, exc_prob_bounds, real_traj)
    ...
    comp_factor = ceil(size(real_traj.conf, 1) / size(exc_prob_bounds, 1));
    ...
    fill(tt, CC, [0, 0, 0.6], ...
        'FaceAlpha', 0.1, ...
        'EdgeColor', 'none')
    stairs(pulse_time, double(real_traj.conf), ...
        'Color', colors(4, :), ...
        'LineWidth', 1.5)
    hold off
    xticklabels([])
    ...
    ylim([0, 3])
    yticks([1, 2])
    lgd = legend(ax2, {'95% conf.', 'Ground truth'}, ...
        'NumColumns', 3, ...
        'FontSize', 11);
    lgd.Layout.Tile = 'North';
    lgd.Box = 'off';
