function accep = sampler_spat_sigmoid(phys, traj, detn, param, hyparam, hmc_param, accep)
    a = 25;
    b = 0.17943;
    new_dim = b * param.confocal_dim ./ param.confocal_dim(1);
    ...
    if log(rand) < log_r
        % accept
        accep(1) = accep(1) + 1;
        traj.update_spat_sigmoid(X, new_dim, true, a, b);
...
function dv = dPSFdX(X, a, b)
    dv = exp(-(a*b)^2*(X+1).^2)-exp(-(a*b)^2*(X-1).^2);
    dv(:, 1:2) = dv(:, 1:2) * a;
    dv(:, 3) = dv(:, 3) * a / 5;
    dv = dv / sqrt(pi);
end

function dv = psf_sigmoid(X, a, b)
    dv = prod(erf(a*b*(1 + X)) + erf(a*b*(1 - X)), 2) / 8;
end